...
//Similarly, write() does not interact directly with the network; it merely put the data 
//into a kernel-side buffer for the TCP stack to consume, and when the buffer is full, write() suspends the calling thread until there is room.
//Readiness notification:
//Non-blocking read
//Non-blocking write:
 //If the data is larger than the available write buffer,
 // a non-blocking write will do a partial write, 
 //while a blocking write may block and wait for the kernel buffer to get clear
 //set the flags with fcntl syscall


 //readineess api 
 //poll
 //APIs will always report a disk file as ready, but the IO will block
 //file IO must be done outside the event loop, in a thread pool,


//store state for connections


//map fd to connections or conn objects use a hashmap or a vector 

//The application code decides the type of readiness notifications. It communicates with the 
//event loop via the want_read & want_write flags in Conn

//the fds argument is then constructed from these flags:
//We monitor all sockets with poll.
//If nothing is happening, we wait.
//If something happens — a new client wants to connect, or an old one wants 
//to send/read — poll tells us exactly who wants what.
//We respond to it. Then repeat.
//This way, we efficiently serve many clients without wasting CPU, 
//without threads, and only reacting when needed.

// handle the listening socket
//if (poll_args[0].revents) {

//Call the application code if they are ready for IO.
//event loop calls back the application code to do accept().
//This is not universally true. For example, some proxies and messaging 
//protocols are not request-response and can read and write simultaneously.
